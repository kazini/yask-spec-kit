...
get_project_root() {
    ...
    while [ "$current_dir" != "/" ]; do
        if [ -d "$current_dir/blueprint" ]; then
            echo "$current_dir"
            return 0
        fi
...
init_project() {
    ...
    mkdir -p blueprint/{specs,templates,docs}
    
    if [ -d "$SCRIPT_DIR/../templates" ]; then
        cp -r "$SCRIPT_DIR/../templates/"* blueprint/templates/
        echo "✓ Templates copied to blueprint/templates/"
    fi
    
    cat > blueprint/README.md << 'EOF'
# blueprint Directory
...
Use the spec.sh script to create and manage specifications.
EOF
    
    echo "✓ Spec-driven development structure initialized"
    echo "  Use: ./.yask/system/spec.sh new <feature-name> to create your first spec"
}
...
create_spec() {
    ...
    local spec_dir="blueprint/$feature_name"
    ...
    if [ -f "blueprint/templates/requirements-template.md" ]; then
        cp "blueprint/templates/requirements-template.md" "$spec_dir/requirements.md"
    else
...
list_specs() {
    if [ ! -d "blueprint" ]; then
        if [ "$JSON_MODE" = true ]; then
            ...
            echo "No specifications found. Use 'spec.sh init' to initialize."
        fi
        return
    fi
...
validate_spec() {
    ...
    local spec_dir="blueprint/$spec_name"
...
For more information, see the documentation in .yask/
EOF
}
