...
echo "Copying .spec-dev directory to $SPEC_DIR/"
cp -r ./.spec-dev "$SPEC_DIR/" || { echo "ERROR: Failed to copy ./.spec-dev to $SPEC_DIR/"; exit 1; }
...
prompt = 'find main Git folder using (where git) then run this command: <GIT>\bin\bash.exe  <user>\.gemini\commands\spec\spec-dev-setup.sh'
description = "Transform feature ideas into structured specifications through stage-based workflow."
EOF
...
# 1. Copy .spec-dev system files to .gemini/.spec-sys/
echo "Copying .spec-dev system files to $SPEC_SYS_DIR..."
...
if [ -e "./.spec-dev" ]; then # Use -e to check for existence of file or directory
    echo "ERROR: A file or directory named './.spec-dev' already exists in the workspace root."
    echo "Please rename or remove it before running the /spec command to avoid conflicts."
    exit 1
fi

echo "Installing spec-dev prompt as .spec-dev..."
...
echo "📁 System files: $SPEC_SYS_DIR/.spec-dev/"
